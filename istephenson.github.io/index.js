...
function populateRepo(items, id) {
  ...
  // Index fetched repo stats so we can show stars/forks/language
  const repoIndex = new Map((items || []).map(it => [`${it.author}/${it.name}`, it]));
  ...
  const repoTemplate = html`
    <div class="repo-grid">
      ${featured.map((proj) => {
        const key = `${proj.author}/${proj.name}`;
        const stat = repoIndex.get(key);
        return html`
          <article class="case-card">
            <a href="${proj.url}" target="_blank" class="case-link" aria-label="${proj.title}">
              ${proj.image
                ? html`<div class="case-media">
                        <img src="${proj.image}" alt="${proj.title}" class="case-img" loading="lazy" />
                      </div>`
                : html`<div class="case-media case-media--placeholder" aria-hidden="true"></div>`}
              <div class="case-body">
                <h3 class="case-title">${proj.title}</h3>
                <p class="case-desc">${proj.about}</p>
              </div>
              ${statsTemplate(stat)}
            </a>
          </article>
